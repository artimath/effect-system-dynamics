...
import { SimState, SimUnits } from "./Simulation.js";
import { EquationEvaluationError, EquationParseError } from "./internal/equations/errors.js";
import { makeQuantity, divideUnits, equalUnits, } from "./internal/equations/Quantity.js";
import { parseUnitsLiteral } from "./internal/equations/EquationEngine.js";
import { compileEquationGraph, evaluateEquationGraph, EquationGraphCycleError, } from "./internal/equations/GraphEngine.js";
...
const DORMAND_PRINCE_C = [0, 1 / 5, 3 / 10, 4 / 5, 8 / 9, 1, 1];
const DORMAND_PRINCE_A = [
    [],
    [1 / 5],
    [3 / 40, 9 / 40],
    [44 / 45, -56 / 15, 32 / 9],
...
const DORMAND_PRINCE_B5 = [35 / 384, 0, 500 / 1113, 125 / 192, -2187 / 6784, 11 / 84, 0];
const DORMAND_PRINCE_B4 = [5179 / 57600, 0, 7571 / 16695, 393 / 640, -92097 / 339200, 187 / 2100, 1 / 40];
...
const stockUnitsCache = new WeakMap();
const flowUnitsCache = new WeakMap();
const variableGraphCache = new WeakMap();
const delayStateCache = new WeakMap();
const flowEquationCache = new WeakMap();
const stockOrderCache = new WeakMap();
const modelUnitsCache = new WeakMap();
const getDelayStateStore = (model, state) => {
...
const replaceDelayStateStore = (model, store) => {
    delayStateCache.set(model, store);
};
const getStockOrder = (model) => {
...
const getModelUnitMetadata = (unitManager, model) => Effect.gen(function* () {
    const cached = modelUnitsCache.get(model);
    if (cached) {
        return cached;
    }
    const timeUnit = yield* getTimeUnit(unitManager, model);
    const stockUnitsById = new Map();
    const stockNamesById = new Map();
    const stockUnitsRecord = Object.create(null);
    const rateUnitsRecord = Object.create(null);
...
const createZeroRateRecord = (stockOrder) => {
...
const computeEulerStepFast = (stockIds, stocks, rates, dt) => {
    const result = Object.create(null);
...
const blendRK4RatesFast = (stockIds, k1, k2, k3, k4) => {
    const result = Object.create(null);
    for (let index = 0; index < stockIds.length; index += 1) {
        const id = stockIds[index];
        result[id] =
...
const combineRatesFast = (stockIds, base, rates, coefficients, dt) => {
    const result = Object.create(null);
    for (let index = 0; index < stockIds.length; index += 1) {
        const id = stockIds[index];
        let value = base[id] ?? 0;
...
const ensureValidTimeStep = (dt) => dt > 0 && Number.isFinite(dt)
    ? Effect.succeed(undefined)
    : Effect.fail(new InvalidTimeStepError({ dt, min: MIN_TIME_STEP, max: MAX_TIME_STEP }));
const cloneUnitMapRecord = (record) => {
    const result = Object.create(null);
...
const cloneNumberRecord = (record) => {
    const result = Object.create(null);
    for (const [key, value] of Object.entries(record)) {
        result[key] = value;
    }
...
const makeIntermediateState = (base, stocks, variables, time) => new SimState({
    time,
    stocks,
    variables,
    units: base.units,
...
const ensureUnitsKnown = (unitManager, units, context) => unitManager.ensureUnitMap(units).pipe(Effect.catchTag("UnitNotFoundError", (error) => Effect.fail(new EquationEvaluationError({
    expression: context,
    problem: `Unit "${error.symbol}" is not registered`,
}))));
const parseUnitsWithManager = (unitManager, units, context) => Effect.try({
    try: () => {
...
const getStockUnits = (unitManager, stock) => {
    const cached = stockUnitsCache.get(stock);
    if (cached) {
        return Effect.succeed(cached);
    }
    return Effect.tap(parseUnitsWithManager(unitManager, stock.units, `Stock "${stock.name}" units`), (parsed) => Effect.sync(() => {
        stockUnitsCache.set(stock, parsed);
    }));
};
const getFlowDeclaredUnits = (unitManager, flow) => {
    const cached = flowUnitsCache.get(flow);
    if (cached) {
        return Effect.succeed(cached);
    }
    return Effect.tap(parseUnitsWithManager(unitManager, flow.units, `Flow "${flow.name}" units`), (parsed) => Effect.sync(() => {
        flowUnitsCache.set(flow, parsed);
    }));
};
const getFlowEquationAst = (flow) => Effect.try({
    try: () => {
...
const getTimeUnit = (unitManager, model) => Effect.gen(function* () {
...
const getCompiledGraph = (model) => Effect.gen(function* () {
    const cached = variableGraphCache.get(model);
    if (cached) {
        return cached;
    }
    const compiled = yield* compileEquationGraph(model.variables).pipe(Effect.mapError((error) => new EquationEvaluationError({
        expression: model.name,
        problem: error instanceof EquationGraphCycleError
...
const evaluateFlowEquation = (flow, scope, options) => Effect.gen(function* () {
...
const resolveAdaptiveOptions = (overrides) => {
    if (!overrides) {
    ...
    const absoluteToleranceDefault = typeof absoluteTolerance === "number"
        ? absoluteTolerance
        : defaultAdaptiveOptions.absoluteToleranceDefault;
    const relativeToleranceDefault = typeof relativeTolerance === "number"
        ? relativeTolerance
...
const toleranceForStock = (tolerance, fallback, stockId) => {
...
const computeAdaptiveError = (baseStocks, highOrder, lowOrder, options) => {
...
const toSimUnits = (dynamics) => new SimUnits({
...
const advanceState = (state, dt, stocks, end, dynamics) => new SimState({
...
export class Solver extends Context.Tag(solverIdentifier)() {
    ...
    static Euler = Layer.succeed(this, {
        name: "Euler",
        step: (model, state, dt) => Effect.gen(function* () {
            ...
            const dynamics = yield* computeDynamics(unitManager, model, state, workingDelayState, true);
    ...
    static RK4 = Layer.succeed(this, {
        ...
        step: (model, state, dt) => Effect.gen(function* () {
            ...
            const baseDelayState = getDelayStateStore(model, state);
            const workingDelayState = baseDelayState.clone();
            const stage1 = yield* computeDynamics(unitManager, model, state, workingDelayState, false);
            const halfStep = effectiveTimeStep / 2;
            const stage2State = makeIntermediateState(state, computeEulerStepFast(stockOrder, state.stocks, stage1.rates, halfStep), cloneNumberRecord(stage1.variables), Math.min(baseTime + halfStep, end));
            const stage2 = yield* computeDynamics(unitManager, model, stage2State, workingDelayState, false);
            const stage3State = makeIntermediateState(state, computeEulerStepFast(stockOrder, state.stocks, stage2.rates, halfStep), cloneNumberRecord(stage2.variables), Math.min(baseTime + halfStep, end));
            const stage3 = yield* computeDynamics(unitManager, model, stage3State, workingDelayState, false);
            const stage4State = makeIntermediateState(state, computeEulerStepFast(stockOrder, state.stocks, stage3.rates, effectiveTimeStep), cloneNumberRecord(stage3.variables), Math.min(baseTime + effectiveTimeStep, end));
            const stage4 = yield* computeDynamics(unitManager, model, stage4State, workingDelayState, false);
            const blendedRates = blendRK4RatesFast(stockOrder, stage1.rates, stage2.rates, stage3.rates, stage4.rates);
            const nextStocks = computeEulerStepFast(stockOrder, state.stocks, blendedRates, effectiveTimeStep);
            const finalState = makeIntermediateState(state, nextStocks, cloneNumberRecord(stage4.variables), Math.min(baseTime + effectiveTimeStep, end));
            const finalDynamics = yield* computeDynamics(unitManager, model, finalState, workingDelayState, true);
            replaceDelayStateStore(model, workingDelayState);
                    ...
                    while (elapsed + 1e-12 < targetTotal) {
                        ...
                        while (!accepted) {
                            ...
                            if (attempts > resolved.maxAttemptsPerStep || currentStep < resolved.minStep - 1e-12) {
                                return yield* Effect.fail(new ConvergenceError({
                                    model: model.id,
                            ...
                            const stage1 = yield* computeDynamics(unitManager, model, workingState, attemptDelayState, false);
                            const rates = [stage1.rates];
                            const buildStage = (coefficients, timeFraction, previousDynamics) => makeIntermediateState(workingState, combineRatesFast(stockOrder, baseStocks, rates, coefficients, currentStep), cloneNumberRecord(previousDynamics.variables), Math.min(baseTime + timeFraction * currentStep, end));
                            const stage2State = buildStage(DORMAND_PRINCE_A[1], DORMAND_PRINCE_C[1], stage1);
                            const stage2 = yield* computeDynamics(unitManager, model, stage2State, attemptDelayState, false);
                            rates.push(stage2.rates);
                            const stage3State = buildStage(DORMAND_PRINCE_A[2], DORMAND_PRINCE_C[2], stage2);
                            const stage3 = yield* computeDynamics(unitManager, model, stage3State, attemptDelayState, false);
                            rates.push(stage3.rates);
                            const stage4State = buildStage(DORMAND_PRINCE_A[3], DORMAND_PRINCE_C[3], stage3);
                            const stage4 = yield* computeDynamics(unitManager, model, stage4State, attemptDelayState, false);
                            rates.push(stage4.rates);
                            const stage5State = buildStage(DORMAND_PRINCE_A[4], DORMAND_PRINCE_C[4], stage4);
                            const stage5 = yield* computeDynamics(unitManager, model, stage5State, attemptDelayState, false);
                            rates.push(stage5.rates);
                            const stage6State = buildStage(DORMAND_PRINCE_A[5], DORMAND_PRINCE_C[5], stage5);
                            const stage6 = yield* computeDynamics(unitManager, model, stage6State, attemptDelayState, false);
                            rates.push(stage6.rates);
                            const stage7State = buildStage(DORMAND_PRINCE_A[6], DORMAND_PRINCE_C[6], stage6);
                            const stage7 = yield* computeDynamics(unitManager, model, stage7State, attemptDelayState, false);
                            rates.push(stage7.rates);
                            const highOrderStocks = combineRatesFast(stockOrder, baseStocks, rates, DORMAND_PRINCE_B5, currentStep);
                            const lowOrderStocks = combineRatesFast(stockOrder, baseStocks, rates, DORMAND_PRINCE_B4, currentStep);
                            ...
                            if (errorRatio <= 1) {
                                const finalStateCandidate = makeIntermediateState(workingState, highOrderStocks, cloneNumberRecord(stage7.variables), Math.min(baseTime + currentStep, end));
                                const finalDynamics = yield* computeDynamics(unitManager, model, finalStateCandidate, attemptDelayState, true);
                                ...
                                nextStep = clampNumber(currentStep * boundedScale, resolved.minStep, resolved.maxStep);
                                accepted = true;
                            }
                            else {
                                ...
                                currentStep = clampNumber(Math.max(proposed, resolved.minStep), resolved.minStep, Math.min(resolved.maxStep, remaining));
                            }
                        }
                    }
...
const isUnitlessMap = (units) => Object.keys(units).length === 0;
...
const buildScope = (model, state, stockUnitsById, timeUnit) => {
    ...
    const assignAliases = (quantity, aliases) => {
...
const validateFlowQuantity = (flow, quantity, declaredUnits, stockUnitsById, stockNamesById, timeUnit, rateUnitsByStockId) => Effect.gen(function* () {
    const attached = [];
    if (flow.source) {
        const sourceUnits = stockUnitsById.get(flow.source);
        if (!sourceUnits) {
            yield* Effect.fail(new EquationEvaluationError({
                expression: flow.rateEquation,
...
const addRateContribution = (rates, rateUnits, stockId, rateQuantity, multiplier, flow, stockNamesById) => Effect.gen(function* () {
...
const computeDynamics = (unitManager, model, state, delayState, commitDelay) => Effect.gen(function* () {
    const metadata = yield* getModelUnitMetadata(unitManager, model);
    const { timeUnit, stockUnitsById, stockNamesById, stockUnitsRecord, rateUnitsRecord, stockOrder, } = metadata;
    ...
    for (const flow of model.flows) {
        ...
        yield* validateFlowQuantity(flow, rateQuantity, declaredUnits, stockUnitsById, stockNamesById, timeUnit, rateUnitsRecord);
        if (flow.source) {
            yield* addRateContribution(numericRates, rateUnitsRecord, flow.source, rateQuantity, -1, flow, stockNamesById);
        }
        if (flow.target) {
            yield* addRateContribution(numericRates, rateUnitsRecord, flow.target, rateQuantity, 1, flow, stockNamesById);
        }
    }
    return {
        rates: numericRates,
